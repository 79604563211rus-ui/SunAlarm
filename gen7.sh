...
cat > app/src/main/java/com/example/sunalarm/SunTheme.java <<'EOF'
package com.example.sunalarm;

import android.graphics.Color;

/** Палитры и типы сцен для тем рассвета/заката. Без сторонних зависимостей. */
public final class SunTheme {
    public static final int HILLS = 0;
    public static final int SEA = 1;
    public static final int DESERT = 2;
    public static final int POLAR = 3;

    public static final int SCENE_HILLS = 0;
    public static final int SCENE_SEA = 1;
    public static final int SCENE_DUNES = 2;
    public static final int SCENE_ICE = 3;

    public static final int COUNT = 4;

    private SunTheme() {}

    public static int scene(int t) {
        switch (t) {
            case SEA: return SCENE_SEA;
            case DESERT: return SCENE_DUNES;
            case POLAR: return SCENE_ICE;
            default: return SCENE_HILLS;
        }
    }

    /** Высота горизонта (доля высоты), из-за которого встаёт солнце. */
    public static float horizonFrac(int t) {
        switch (scene(t)) {
            case SCENE_SEA: return 0.60f;
            case SCENE_DUNES: return 0.72f;
            case SCENE_ICE: return 0.74f;
            default: return 0.80f;
        }
    }

    private static final int[] DAY_TOP  = {0xFF7EC3E0, 0xFF4FA3D1, 0xFFF6A96B, 0xFF9FD8E8};
    private static final int[] DAY_BOT  = {0xFFFFD180, 0xFFBFE3F0, 0xFFFFE0A3, 0xFFE8F6FA};
    private static final int[] NIGHT_TOP= {0xFF061024, 0xFF04122B, 0xFF1A1030, 0xFF021018};
    private static final int[] NIGHT_BOT= {0xFF23304F, 0xFF0E3A5C, 0xFF4A2A4A, 0xFF0A2A3A};

    private static final int[] DAY_FAR  = {0xFF4A6B3A, 0xFF1E6FA0, 0xFFE0A35C, 0xFFBFE6EC};
    private static final int[] DAY_NEAR = {0xFF2E4A26, 0xFF0E4A78, 0xFFC8823C, 0xFF8FC6D2};
    private static final int[] NIGHT_FAR = {0xFF10203A, 0xFF06243F, 0xFF3A2540, 0xFF123040};
    private static final int[] NIGHT_NEAR= {0xFF0A1730, 0xFF03162A, 0xFF241430, 0xFF08202C};

    public static int dayTop(int t){return DAY_TOP[idx(t)];}
    public static int dayBot(int t){return DAY_BOT[idx(t)];}
    public static int nightTop(int t){return NIGHT_TOP[idx(t)];}
    public static int nightBot(int t){return NIGHT_BOT[idx(t)];}
    public static int dayFar(int t){return DAY_FAR[idx(t)];}
    public static int dayNear(int t){return DAY_NEAR[idx(t)];}
    public static int nightFar(int t){return NIGHT_FAR[idx(t)];}
    public static int nightNear(int t){return NIGHT_NEAR[idx(t)];}

    private static int idx(int t){ return (t < 0 || t >= COUNT) ? 0 : t; }
...
/** Анимированная сцена: солнце/луна, звёзды с мерцанием, падающие звёзды, 4 темы переднего плана. */
public class SunView extends View {
    ...
    protected void onDraw(Canvas c){
        ...
        float sunY = h*1.08f + (h*0.18f - h*1.08f) * t;
        float sunX = w/2f;
        ...
        // Луна (видна ночью)
        float moonA = Math.max(0f, Math.min(1f, (nightF - 0.45f)/0.4f));
        if (moonA > 0.01f){
            float mx = w*0.76f, my = h*0.20f, mr = Math.min(w,h)*0.07f;
            moon.setAlpha((int)(moonA*255f));
            moon.setColor(0xFFE8EEF5);
            c.drawCircle(mx, my, mr*1.8f, glowMoon(mx, my, mr*1.8f, (int)(moonA*0x40)));
            c.drawCircle(mx, my, mr, moon);
            crater.setAlpha((int)(moonA*0x55));
            c.drawCircle(mx - mr*0.3f, my - mr*0.2f, mr*0.22f, crater);
            c.drawCircle(mx + mr*0.25f, my + mr*0.25f, mr*0.16f, crater);
            c.drawCircle(mx + mr*0.1f, my - mr*0.35f, mr*0.12f, crater);
            moon.setAlpha(255); crater.setAlpha(0x33A9B4C2);
        }

        // Передний план по теме
    ...
    private Shader glowMoon(float x, float y, float r, int a){
        return new RadialGradient(x, y, r,
                new int[]{withAlpha(0xFFE8EEF5, a/255f), 0x00000000},
